...
export default function BattleArena() {
  ...
  useEffect(() => {
    ...
    try {
      ws = new WebSocket(
        `${BASE_URL.replace(/^http/, "ws")}/ws/room/${roomCode}`,
      );
      ws.onmessage = (e) => {
        try {
    ...
    function startFallback() {
      fallback = setInterval(() => {
        if (Math.random() > 0.75) {
          setOpponentScore((p) => {
            const next = p + Math.floor(Math.random() * 60 + 10);
            setOppScorePulse(true);
            setTimeout(() => setOppScorePulse(false), 600);
            return next;
          });
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      toast(
        `Executed — ${results.length} test case${results.length !== 1 ? "s" : ""} run`,
      );
    } catch (err: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result: { status: "AC" | "WA" | "TLE"; points: number } =
        await apiSubmit(code, problem.id, roomCode);
      setSubmissions((prev) => [
        { time: elapsed(), problem: problem.title, status: result.status },
        ...prev,
      ]);
      if (result.status === "AC") {
        setMyScore((prev) => {
        ...
        toast.error("⏱ Time Limit Exceeded", {
          description: "Your solution was too slow.",
        });
      } else {
